...
import { FiSearch, FiCalendar, FiLogIn, FiLogOut, FiRotateCcw } from "react-icons/fi";
import toast from "react-hot-toast";
import { fetchHotelBookings, checkInBooking, checkOutBooking, refundPayment } from "../../Services/manager.service";
...
const ManagerBookings = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      const { data, error } = await fetchHotelBookings({
        page,
        limit: PAGE_SIZE,
  ...
  }, [page, search, bookingStatus, paymentStatus, refreshKey]);

  const runAction = async (booking, action) => {
    setActionLoading(booking._id);
    const handlers = { checkIn: checkInBooking, checkOut: checkOutBooking, refund: refundPayment };
    const { data, error } = await handlers[action]({ bookingId: booking._id });
  ...
  const actionClass = "px-3 py-1.5 rounded-lg transition-colors text-xs font-semibold flex items-center gap-1 disabled:opacity-50";
  ...
  return (
    <div className="animate-in fade-in zoom-in duration-500 relative">
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-stone-400">Verify arrivals, check guests in and out, and process refunds for cancelled paid stays.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 md:flex md:gap-3">
        <div className="relative col-span-2 flex-1 md:max-w-sm">
          ...
          <input
            ...
            placeholder="Search guest, room or offer code..."
                ...
                {bookings.map((b) => (
                  <tr key={b._id} className="hover:bg-white/5 transition-colors">
                    <td className="py-4 px-6">
                      ...
                      <p className="text-[11px] font-mono text-stone-500">{b._id.slice(-8).toUpperCase()}</p>
                    </td>
                    <td className="py-4 px-6">
                      <p className="font-mono text-white">{b.roomNumber}</p>
                      <p className="text-xs text-stone-400 capitalize">{b.roomType}</p>
                    </td>
                    <td className="py-4 px-6 text-xs">
                      ...
                      <p className="text-stone-500 mt-0.5">
                        {b.numberOfNights} night{b.numberOfNights === 1 ? "" : "s"} · {b.adults} adult{b.adults === 1 ? "" : "s"}{b.children ? `, ${b.children} child${b.children === 1 ? "" : "ren"}` : ""}
                        {b.bookingStatus === "booked" && isToday(b.checkInDate) && <span className="ml-2 rounded-full bg-amber-500/20 px-2 py-0.5 text-[10px] font-bold text-amber-300">ARRIVING TODAY</span>}
                        {b.bookingStatus === "checkedIn" && isToday(b.checkOutDate) && <span className="ml-2 rounded-full bg-blue-500/20 px-2 py-0.5 text-[10px] font-bold text-blue-300">LEAVING TODAY</span>}
                      </p>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-2">
                        {b.bookingStatus === "booked" && (
                          <ConfirmButton
                            disabled={actionLoading === b._id || b.paymentStatus !== "paid"}
                            onConfirm={() => runAction(b, "checkIn")}
                            confirmLabel="Confirm check-in"
                            className={`${actionClass} bg-blue-500/20 text-blue-300 hover:bg-blue-500/30`}
                            armedClassName="!bg-blue-500 !text-white"
                          >
                            <FiLogIn className="h-3 w-3" /> {b.paymentStatus === "paid" ? "Check in" : "Payment pending"}
                          </ConfirmButton>
                        )}
                        {b.bookingStatus === "checkedIn" && (
                          <ConfirmButton
                            disabled={actionLoading === b._id}
                            onConfirm={() => runAction(b, "checkOut")}
                            confirmLabel="Confirm check-out"
                            className={`${actionClass} bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30`}
                            armedClassName="!bg-emerald-500 !text-white"
                          >
                            <FiLogOut className="h-3 w-3" /> Check out
                          </ConfirmButton>
                        )}
                        {b.bookingStatus === "cancelled" && b.paymentStatus === "paid" && (
                          <ConfirmButton
                            disabled={actionLoading === b._id}
                            onConfirm={() => runAction(b, "refund")}
                            confirmLabel="Confirm refund"
                            className={`${actionClass} bg-purple-500/20 text-purple-300 hover:bg-purple-500/30`}
                            armedClassName="!bg-purple-500 !text-white"
                          >
                            <FiRotateCcw className="h-3 w-3" /> Refund
                          </ConfirmButton>
                        )}
                        {((b.bookingStatus === "checkedOut") || (b.bookingStatus === "cancelled" && b.paymentStatus !== "paid")) && (
                          <span className="text-xs text-stone-500">No action</span>
                        )}
                      </div>
                    </td>
                  </tr>
